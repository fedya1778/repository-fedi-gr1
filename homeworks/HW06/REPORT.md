...
> Важно: не изменяйте названия разделов. Заполняйте только текст и результаты.
...
1. **Ансамблевые методы превосходят простые модели** — RandomForest показал наилучшие результаты, что согласуется с теоретическими ожиданиями — ансамбли устойчивы к переобучению и эффективно работают с нелинейными зависимостями.
2. **Корректный ML-протокол критически важен** — использование фиксированных `random_state`, `stratify`, `cv=3`, стратификации, одинаковых метрик и единой тестовой выборки обеспечивает объективное сравнение моделей.
3. **Базовые модели обязательны для анализа** — Dummy и LogisticRegression задали разумный ориентир, относительно которого ансамблевые методы продемонстрировали существенное улучшение.
