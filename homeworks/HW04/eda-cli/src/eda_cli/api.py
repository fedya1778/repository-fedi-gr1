...
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    ...
    # Вызываем все функции из HW03 
    _ = summarize_dataset(df)      # можно не использовать, но вызываем
    _ = missing_table(df)          # можно не использовать, но вызываем
    flags = compute_quality_flags(df)

    # Формируем ответ ТОЛЬКО с флагами
